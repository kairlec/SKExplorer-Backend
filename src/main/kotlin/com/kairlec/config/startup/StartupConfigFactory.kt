...
open class StartupConfigFactory {
    ...
    open fun startupConfigMaker(): StartupConfig {
        ...
        val dataDirPathString = env["data"] ?: "data"
        val dataDirPath: Path
        try {
            dataDirPath = Path.of(dataDirPathString)
        } catch (e: InvalidPathException) {
            e.printStackTrace()
            logger.error(e.message)
            logger.fatal("config配置了无效的路径:$dataDirPathString")
            exitProcess(-1)
        }
        return StartupConfig(contentDir, excludeDir, excludeFile, excludeExt, captchaCount, dataDirPath)
    }
