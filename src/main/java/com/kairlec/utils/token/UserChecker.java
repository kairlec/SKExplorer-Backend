...
public class UserChecker {
    ...
    @Setter(AccessLevel.NONE)
    private ServiceError failedResult;
    ...
    public boolean check() {
        ...
        if (session.getAttribute("user") != null) {
            failedResult = ServiceError.HAD_LOGGED_IN;//已经登录过
            return false;
        }
        if (username == null) {
            failedResult = ServiceError.NULL_USERNAME;//空的用户名
            return false;
        }
        if (password == null) {
            failedResult = ServiceError.NULL_PASSWORD;//空的密码
            return false;
        }
        User user = LocalConfig.getUserService().getUser(username);
        if (user == null) {
            failedResult = ServiceError.USERNAME_NOT_EXISTS;//错误的用户名
            return false;
        }
        ...
        if (captcha != null) {
            ...
            if (captchaString == null) {
                failedResult = ServiceError.NULL_CAPTCHA;//需要验证但是验证码为空
                return false;
            }
            if (!captcha.check(captchaString)) {
                failedResult = ServiceError.WRONG_CAPTCHA;//错误的验证码
                return false;
            }
            ...
            if (IP != null && user.getIP() != null && !IP.equals(user.getIP())) {
                ...
                failedResult = ServiceError.NEED_VERIFY;//不受信任的IP,需要验证
                return false;
            }
        }
        ...
        try {
            ...
            failedResult = ServiceError.UNKNOWN_PASSWORD;//未知的密码串
            return false;
        }
        ...
        if (!user.getPassword().equals(password)) {
            session.setAttribute("captcha", CaptchaMaker.getCaptcha(LocalConfig.getConfigBean().getCaptchacount()));
            failedResult = ServiceError.WRONG_PASSWORD;//错误的密码
            return false;
        }
        return true;
    }
